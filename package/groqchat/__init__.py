...
def checkPyaudio():
    try:
        ...
        if config.isTermux:
            ...
            config.pyaudioInstalled = (
                True if installPipPackage("--upgrade PyAudio") else False
            )
...
def voiceTyping():
    ...
    with sr.Microphone() as source:
        if config.voiceTypingNotification:
            playAudio(
                os.path.join(config.packageFolder, "audio", "notification1_mild.mp3")
            )
    ...
    if config.voiceTypingPlatform == "google":
        ...
        and "Speech-to-Text" in config.enabledGoogleAPIs
    ):
        # recognize speech using Google Cloud Speech
        try:
            # check availabl languages at: https://cloud.google.com/speech-to-text/docs/speech-to-text-supported-languages
            # config.voiceTypingLanguage should be code list in column BCP-47 at https://cloud.google.com/speech-to-text/docs/speech-to-text-supported-languages
            return r.recognize_google_cloud(
                audio,
                language=config.voiceTypingLanguage,
                credentials_json=config.google_cloud_credentials,
            )
        except sr.UnknownValueError:
            # return "[Speech unrecognized!]"
            return ""
        ...
        try:
            ...
            return r.recognize_whisper(
                audio,
                model=(
                    config.voiceTypingWhisperEnglishModel
                    if config.voiceTypingLanguage == "english"
                    else "large"
                ),
                language=config.voiceTypingLanguage,
            )
        ...
        if not os.path.isfile(config.whispercpp_main) or not os.path.isfile(
            config.whispercpp_model
        ):
            return "[Error]"
        ...
        process = subprocess.Popen(
            cli.rstrip(), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def wrapText(content, terminal_width=None):
    ...
    return "\n".join(
        [textwrap.fill(line, width=terminal_width) for line in content.split("\n")]
    )
...
def print2(content):
    print_formatted_text(
        HTML(
            f"<{config.terminalPromptIndicatorColor2}>{content}</{config.terminalPromptIndicatorColor2}>"
        )
    )
...
def installPipPackage(module, update=True):
    ...
    if isCommandInstalled("pip"):
        ...
        if update:
            if not config.isPipUpdated:
                ...
                try:
                    ...
                    updatePip = subprocess.Popen(
                        f"{pipInstallCommand} --upgrade pip",
                        shell=True,
        ...
        try:
            ...
            installNewModule = subprocess.Popen(
                f"{pipInstallCommand} {module}",
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
            *_, stderr = installNewModule.communicate()
            if not stderr:
                print(
                    f"Package '{moduleName}' {'upgraded' if upgrade else 'installed'}!"
                )
            else:
                print(
                    f"Failed {'upgrading' if upgrade else 'installing'} package '{moduleName}'!"
                )
                if config.developer:
                    print(stderr)
            return True
