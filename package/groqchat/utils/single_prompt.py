    ...
    def run(
        ...
        **kwargs,
        ...
        def _(_):
            ...
            run_in_terminal(
                lambda: print3(
                    f"Word Wrap: '{'enabled' if config.wrapWords else 'disabled'}'!"
                )
            )
        ...
        def _(_):
            if config.tts:
                ...
                run_in_terminal(
                    lambda: print3(
                        f"Response Audio: '{'enabled' if config.ttsOutput else 'disabled'}'!"
                    )
                )
        ...
        def _(event):
            if config.pyaudioInstalled:
                buffer = event.app.current_buffer
                buffer.text = (
                    f"{buffer.text}{' ' if buffer.text else ''}{voiceTyping()}"
                )
                ...
                run_in_terminal(
                    lambda: print2("Install PyAudio first to enable voice entry!")
                )
        ...
        userInput = inputPrompt(
            ...
            bottom_toolbar=(
                ...
                else f""" {str(config.hotkey_exit).replace("'", "")} {config.exit_entry}"""
            ),
            # enable_system_prompt=True,
            swap_light_and_dark_colors=Condition(
                lambda: not config.terminalResourceLinkColor.startswith("ansibright")
            ),
            style=style,
            validator=validator,
            multiline=Condition(
                lambda: hasattr(config, "currentMessages") and config.multilineInput
            ),
            default=default,
        ...
        return (
            userInput
            if hasattr(config, "addPathAt") and config.addPathAt
            else userInput.strip()
        )
