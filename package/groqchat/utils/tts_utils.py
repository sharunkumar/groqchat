...
import os, traceback, pydoc, shutil
        ...
        if config.tts and content.strip():
            try:
                if shutil.which("termux-tts-speak"):
                    ...
                    additional_options = (
                        f" {config.termuxtts_additional_options.strip()}"
                        if config.termuxtts_additional_options.strip()
                        else ""
                    )
                    voice = (
                        f" -l {config.termuxtts_voice.strip()}"
                        if config.termuxtts_voice.strip()
                        else ""
                    )
                    ...
                    additional_options = (
                        f" {config.say_additional_options.strip()}"
                        if config.say_additional_options.strip()
                        else ""
                    )
                    voice = (
                        f" -v {config.say_voice.strip()}"
                        if config.say_voice.strip()
                        else ""
                    )
                    ...
                    if not isWsayFound:
                        ...
                        downloadFile(
                            "https://github.com/p-groarke/wsay/releases/download/1.6.2/wsay.exe",
                            homeWsay,
                        )
                        isWsayFound = shutil.which("wsay") or os.path.isfile(homeWsay)
                    ...
                    additional_options = (
                        f" {config.wsay_additional_options.strip()}"
                        if config.wsay_additional_options.strip()
                        else ""
                    )
                    ...
                    model_dir = os.path.join(
                        config.localStorage, "gchat", "LLMs", "piper"
                    )
                    model_path = (
                        f"""{os.path.join(model_dir, config.piper_voice)}.onnx"""
                    )
                    model_config_path = f"""{model_path}.json"""
                    piper_additional_options = (
                        f" {config.piper_additional_options.strip()}"
                        if config.piper_additional_options.strip()
                        else ""
                    )
